...
internal class Cone2ContactReaderAdapter(val contactInterface: ContactInterface) :
    ReaderSpi, ConfigurableReaderSpi, Cone2ContactReader {
  ...
  override fun openPhysicalChannel() {
    try {
      ...
      val result = reader.cscSelectSam(contactInterface.slotId, currentSamProtocol!!)
      if (result != RCSC_Ok) {
  ...
  override fun deactivateProtocol(readerProtocol: String?) {
    ...
    if (!readerProtocol.equals(currentSamProtocolName)) {
      throw IllegalStateException(
          "$readerProtocol is not currently activated. Current SAM protocol is : $currentSamProtocolName")
    } else {
  ...
  override fun checkCardPresence(): Boolean {
    return try {
      ...
      currentSamProtocol?.let {
        val result = reader.cscSelectSam(contactInterface.slotId, it)
        result == RCSC_Ok
      }
          ?: false
    } finally {
